...
object Huffman {
   ...
   */
    ...
    chars.groupBy(ch=>ch).mapValues(_.size).toList
  }
   ...
   */
    ...
    //freqs.map(eachValue=>eachValue._2).sorted
   ...
   */
    def combine(trees: List[CodeTree]): List[CodeTree] = trees  match{
      case left::right::restOfTheTree =>(makeCodeTree(left,right)::restOfTheTree).sortBy(t=>weight(t))
      case _ =>trees
  }
   ...
   */
    def until(singletonList:List[CodeTree] => Boolean, combine: List[CodeTree]=>List[CodeTree])(trees:List[CodeTree]): List[CodeTree]= {
   ...
   */
  def createCodeTree(chars: List[Char]) = {
